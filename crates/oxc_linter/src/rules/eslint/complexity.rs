...
impl Default for ComplexityConfig {
    fn default() -> Self {
        Self { max: 0, variant: ComplexityVariant::Classic }
    }
}
...
mod tests {
    ...
    fn test_compute_complexity_directly() {
        ...
        let test_cases = vec![
            ...
            (
                "function oneIfElseIf(a, b) { if (a) { return 1; } else if (b) { return 2; } return 0; }",
                2,
            ),
            ...
            (
                "function switchWithCases(val) { switch(val) { case 1: return 'one'; case 2: return 'two'; } }",
                1,
            ),
            (
                "function switchWithDefault(val) { switch(val) { case 1: return 'one'; case 2: return 'two'; default: return 'other'; } }",
                2,
            ),
            ...
            (
                "function twoRepeatingLogicalSequences(a, b, c, d, e){ if(a && b && c || d || e) {}}",
                3,
            ), // +1 for if; +1 for all &&; +1 for all ||
            ...
            (
                "function multipleRecursions(n) { return multipleRecursions(n - 1) + multipleRecursions(n - 2); }",
                2,
            ),
    ...
    fn test_from_configuration() {
        let from_number = Complexity::from_configuration(serde_json::json!([5])).unwrap();
        assert_eq!(from_number.max, 5);
        assert_eq!(from_number.variant, ComplexityVariant::Classic);

        let from_max = Complexity::from_configuration(serde_json::json!([{ "max": 7 }])).unwrap();
        assert_eq!(from_max.max, 7);
        assert_eq!(from_max.variant, ComplexityVariant::Classic);

        let from_maximum =
            Complexity::from_configuration(serde_json::json!([{ "maximum": 9 }])).unwrap();
        assert_eq!(from_maximum.max, 9);
        assert_eq!(from_maximum.variant, ComplexityVariant::Classic);

        let from_variant =
            Complexity::from_configuration(serde_json::json!([{ "variant": "modified" }])).unwrap();
        assert_eq!(from_variant.max, 0);
        assert_eq!(from_variant.variant, ComplexityVariant::Modified);
    }
